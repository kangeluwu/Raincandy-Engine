...
class CreditsStateNEW extends MusicBeatState
{
  ...
  function buildCreditsGroup():Void
  {
    ...
    for (entry in credits.CreditsDataHandler.CREDITS_DATA.entries)
    {
      ...
      {
        var backers = credits.CreditsDataHandler.fetchBackerEntries();
  ...
  function buildCreditsLine(text:String, yPos:Float, header:Bool, side:CreditsSide = CreditsSide.Center):FlxText
  {
    ...
    creditsLine.setFormat(Paths.font(CREDITS_FONT), size, CREDITS_FONT_COLOR, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, CREDITS_FONT_STROKE_COLOR, true);
