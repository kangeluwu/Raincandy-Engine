...
class CreditsDataHandler
{
  ...
  {
    #if MODS_ALLOWED
    if (!FNFAssets.exists(CREDITS_DATA_PATH))
      CREDITS_DATA_PATH = Paths.modFolders('data/credits.json');
#end
    #if !macro
  ...
  static function printErrors(errors:Array<json2object.Error>, id:String = ''):Void
  {
    ...
    for (error in errors)
      customlize.DataError.printError(error);
  }
  #end
}
