...
class StickerSubState extends MusicBeatSubstate
{
  ...
  public function new(?oldStickers:Array<StickerSprite>, ?targetState:StickerSubState->FlxState):Void
  {
    ...
    var stickpush:Array<String> = [];
		var checkurfol:Array<String> = [SUtil.getPath() + Paths.getPreloadPath('sounds/stickersounds/'),SUtil.getPath() + Paths.getPreloadPath('shared/sounds/stickersounds/')];

		#if MODS_ALLOWED
		checkurfol.insert(0, Paths.mods('sounds/stickersounds/'));
		if(Paths.currentModDirectory != null && Paths.currentModDirectory.length > 0)
			checkurfol.insert(0, Paths.mods(Paths.currentModDirectory + '/sounds/stickersounds/'));

		for(mod in Paths.getGlobalMods())
			checkurfol.insert(0, Paths.mods(mod + '/sounds/stickersounds/'));
		#end

		for (folder in checkurfol)
		{
			if(sys.FileSystem.exists(folder))
			{
				for (file in sys.FileSystem.readDirectory(folder))
				{
					if(file.endsWith(Paths.SOUND_EXT) && !stickpush.contains(file))
					{
            sounds.push(file);
						stickpush.push(file);
					}
  ...
  public function degenStickers():Void
  {
    ...
    for (ind => sticker in grpStickers.members)
    {
      new FlxTimer().start(sticker.timing, _ -> {
        ...
        FlxG.sound.play(Paths.sound('stickersounds/$daSound'));
  ...
  function regenStickers():Void
  {
    ...
    while (xPos <= FlxG.width)
    {
      ...
      for (key in stickers.keys())
        idk.push(key);
      var stickerSet:String = FlxG.random.getObject(idk);
...
class StickerSprite extends DynamicSprite
{
...
class StickerInfo
{
  ...
  {
    var path = 'windose_data/images/transitionSwag/' + stickerSet + '/stickers.json';
    if (!FNFAssets.exists(path))
      path = 'windose_data/shared/images/transitionSwag/' + stickerSet + '/stickers.json';
    #if MODS_ALLOWED
    if (!FNFAssets.exists(path))
      path = Paths.modFolders('images/transitionSwag/' + stickerSet + '/stickers.json');
    #end
    var json = Json.parse(FNFAssets.getText(path));
