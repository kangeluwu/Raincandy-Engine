...
class LoadingState extends MusicBeatState
{
	...
	static var imagesToPrepareEX:Array<String> = [];
	static var soundsToPrepare:Array<String> = [];
	static var soundsToPrepareEX:Array<String> = [];
	...
	public static function prepareEX(images:Array<String> = null, sounds:Array<String> = null)
		{
			if (images != null) imagesToPrepareEX = imagesToPrepareEX.concat(images);
			if (sounds != null) soundsToPrepareEX = soundsToPrepareEX.concat(sounds);
	...
	public static function prepareToSong()
	{
		...
		Thread.create(() -> {
			...
			var stageDataHSCRIPT = Paths.mergeAllTextsNamed('stage/custom_Hscript_stages/custom_stages.json','');
			for (stages in stageDataHSCRIPT){
				var json = CoolUtil.parseJson(FNFAssets.getText(stages));
				var imgsEX:Array<String> = [];
				var sndsEX:Array<String> = [];
				if (Reflect.hasField(json,song.stage))
				{
					var f =Paths.mergeAllTextsNamed('stage/custom_Hscript_stages/'+Reflect.field(json,song.stage)+'/','');
					for (asset in FileSystem.readDirectory(f))
					{
						if(asset.endsWith('.png'))
							imgsEX.push(f + asset.substr(0,'.png'.length));
						else if(asset.endsWith('.ogg'))
							sndsEX.push(f + asset.substr(0,'.ogg'.length));
					}
					prepareEX(imgsEX, sndsEX);
				}
			}
			songsToPrepare.push('$folder/'+song.songFileNames[0]);

			var player1:String = song.player1;
			var player2:String = song.player2;
			var gfVersion:String = song.gfVersion;
			var prefixVocals:String = song.needsVoices ? '$folder/'+song.songFileNames[1] : null;
			var pV:Array<String> = song.playerVocalFiles != null ? song.playerVocalFiles : null;
			var OV:Array<String> = song.opponentVocalFiles != null ? song.opponentVocalFiles : null;
			...
			{	
				if (pV != null && oV != null){
					for (p in pv){
				if(Paths.fileExists(prefixVocals +'-' + p + '.${Paths.SOUND_EXT}', SOUND, false, 'songs'))
				{
					songsToPrepare.push(prefixVocals +'-' + p);
				}
			}
			for (o in oV){
				if(Paths.fileExists(prefixVocals +'-' + o + '.${Paths.SOUND_EXT}', SOUND, false, 'songs'))
				{
					songsToPrepare.push(prefixVocals +'-' + o);
				}
			}
			}
	...
	{
		clearInvalidFrom(imagesToPrepareEX, '', '.png', IMAGE);
		clearInvalidFrom(imagesToPrepare, 'images', '.png', IMAGE);
		clearInvalidFrom(soundsToPrepare, 'sounds', '.${Paths.SOUND_EXT}', SOUND);
		clearInvalidFrom(musicToPrepare, 'music',' .${Paths.SOUND_EXT}', SOUND);
		clearInvalidFrom(songsToPrepare, 'songs', '.${Paths.SOUND_EXT}', SOUND, 'songs');

		for (arr in [imagesToPrepareEX, imagesToPrepare, soundsToPrepare, musicToPrepare, songsToPrepare])
	...
	static function clearInvalidFrom(arr:Array<String>, prefix:String, ext:String, type:AssetType, ?parentFolder:String = null)
	{
		for (folder in arr.copy())
		{
			...
			if(nam.endsWith('/'))
			{
				for (path in [Paths.getPreloadPath(),Paths.getSharedPath()]){
				for (subfolder in Paths.directoriesWithFile(path, '$prefix/$nam'))
				{
	...
	public static function startThreads()
	{
		...
		for (image in imagesToPrepareEX) initThread(() -> preloadGraphicEX(image), 'image $image');
	}
	...
	static function preloadSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):Null<Sound>
	{
		...
		// thread safe sound loader
		static function preloadSoundEX(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):Null<Sound>
			{
				var file:String = path + '.ogg';
		
				//trace('precaching sound: $file');
				if(!Paths.currentTrackedSounds.exists(file))
				{
					if (#if sys FNFAssets.exists(file) || #end OpenFlAssets.exists(file, SOUND))
					{
						var sound:Sound = #if sys Sound.fromFile(file) #else OpenFlAssets.getSound(file, false) #end;
						mutex.acquire();
						Paths.currentTrackedSounds.set(file, sound);
	...
	static function preloadGraphicEX(key:String):Null<BitmapData>
		{
			try {
				...
				{
					var file:String = Paths.getPath(requestKey, IMAGE);
					if (#if sys FNFAssets.exists(file) || #end OpenFlAssets.exists(file, IMAGE))
					{
						#if sys
						var bitmap:BitmapData = FNFAssets.getBitmapData(file);
						#else
						var bitmap:BitmapData = OpenFlAssets.getBitmapData(file, false);
						#end
						mutex.acquire();
